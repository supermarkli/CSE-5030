...
另外，`BYPASS` 日志反映的是 EX 阶段的旁路选择，而指令日志反映的是 WB 阶段的提交结果，因此两类日志在时间上会自然错开。
...
日志中，`PC=0x0080000020` 的 bypass 来源是 `MEM`，同时 `ld` 与后续 `add` 的 WB 周期分别为 `195` 和 `197`。这说明 load 的结果要到 MEM 阶段才可用，而后一条 `add` 会在更早的 EX 阶段就需要 `t3`，因此产生了 stall。
...
因为 `ld t3, 0(s0)` 的数据不会在 `EX` 阶段立即产生，而是要到 `MEM` 阶段才可用；但紧接着的 `add t4, t3, t0` 会在自己的 `EX` 阶段就读取 `t3`。因此消费者取数时，load 结果还没准备好，必须插入一个 stall cycle。

#### 日志中的哪些证据体现了 stall cycles？

从日志可见，`ld` 位于 `pc=[000000008000001c]`，其对应的 WB 周期是 `C0: 195`；紧随其后的 `add` 位于 `pc=[0000000080000020]`，其对应周期是 `C0: 197`。与前面的普通 ALU 相关相比，这里出现了更明显的间隔。同时，bypass 来源为 `MEM`，也说明 load 的结果直到 MEM 阶段才可被后续指令转发。

### 4.3 Control Hazards

#### 默认的 branch prediction policy 是什么？

从实验日志推断，默认的静态 branch prediction policy 是 `predict not taken`。

#### 你是如何从日志中判断出来的？

从两条前向分支的执行结果可以推断这一点。第一条 `beq a1, a2, pc + 8` 实际 taken，日志显示该分支位于 `pc=[000000008000002c]`，其后进入目标地址 `pc=[0000000080000034]`；这说明顺序路径被纠正，符合“先按 not taken 方向取指，再在分支判定后跳转”的现象。第二条 `beq a4, a5, pc + 12` 实际 not taken，日志中在 `pc=[000000008000003c]` 之后继续执行顺序地址 `pc=[0000000080000040]`，与 `predict not taken` 完全一致。因此可以根据 log 判断默认策略是 `predict not taken`。
